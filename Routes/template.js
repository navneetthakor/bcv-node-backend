...
// add Template
router.post('/add', fetchUser, upload.single('url') ,addTemplate);

//remove template
router.post('/remove', fetchUser,
 [
    body("version", "Provide version Number to remove template").not().isEmpty()
 ]   
,removeTemplate);
...
[
    body("version", "Provide version Number to update").not().isEmpty()
]
,updateTemplate);